...
public class Edge {
    ...
    public boolean intersects(Edge that) {
        ...
        //I don't want to count edges that share an endpoint.
        //That would make most puzzles unsolvable.
        if(this.sharesAnyVertex(that)) {
            return false;
        }
    ...
    private boolean sharesAnyVertex(Edge that) {
        if(this.getStart() == that.getStart()) return true;
        if(this.getStart() == that.getEnd()) return true;
        if(this.getEnd() == that.getEnd()) return true;
        if(this.getEnd() == that.getStart()) return true;
        return false;
    }

    public boolean overlapsAny(LinkedList<Edge> them) {
        Edge that;
    ...
    public int countOverlapsIn(LinkedList<Edge> them) {
        int overlaps = 0;
        Edge that;
        for (int i = 0; i < them.size(); i++) {
            that = them.get(i);
            if (this.intersects(that)) {
                ...
                if (PlanarFrame.DEBUG_OLAP) {
                    System.out.println("The line from "
                            + this.getStart().getName()
                            + " to "
                            + this.getEnd().getName()
                            + " intersects the line from "
                            + that.getStart().getName()
                            + " to "
                            + that.getEnd().getName() + ".");
                }
            }
    ...
    public LinkedList<Edge> allOverlapsIn(LinkedList<Edge> them) {
